...
def predict():
    ...
    model = joblib.load('covid19_prediction_model.ml')
   
    a = [float(x) for x in request.form.values()]
    arr = [np.array(a)]
   # arr = np.array([a])
    output_predict = model.predict(arr)
    return render_template('index.html',prediction_text='The Severity Condition based on the Covid-19 Symptoms: {}'.format(output_predict))
